...
public class OrderController {
    ...
    @Operation(summary = "Get the product with the highest price on a specific date")
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "404", description = "No orders for the selected date", content = @Content)
    })
    @GetMapping("/by-price-and-date")
    public ResponseEntity<List<Order>> findByPriceAndDate(@RequestParam LocalDate date){
        List<Order> response = orderService.findByPriceAndDate(date);
